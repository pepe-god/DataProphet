...
def read_config():
    ...
    return config['DATABASE'], config['GSMDATA'], config['ADRESSDATA']

def validate_tc(tc_no):
    return len(tc_no) == 11 and tc_no.isdigit()

def write_person_info(writer, person, category, gsm_data=None, adres_data=None):
    writer.writerow([category] + [str(value) for value in person[1:]] + ([gsm_data] if gsm_data else []) + ([adres_data] if adres_data else []))
...
def get_adres_data_by_tc(cursor, tc_no):
    query = "SELECT GUNCELADRES, GECMISADRES FROM `adresv2` WHERE `TC` = %s"
    result = execute_query(cursor, query, (tc_no,))
    return result[0] if result else ("Yok", "Yok")
...
def process_family_tree(cursor, gsm_cursor, adres_cursor, tc_no, writer, prefix=""):
    main_person = get_family_member_by_tc(cursor, tc_no)
    if not main_person:
        return

    gsm_data = get_gsm_data_by_tc(gsm_cursor, tc_no)
    adres_data = get_adres_data_by_tc(adres_cursor, tc_no)
    write_person_info(writer, main_person, prefix + "Ana Kayıt", gsm_data, adres_data)
    ...
    if anne_result:
        ...
        adres_data = get_adres_data_by_tc(adres_cursor, anne_tc)
        write_person_info(writer, anne_result, prefix + "Anne", gsm_data, adres_data)
        ...
        if anne_anne_result:
            ...
            adres_data = get_adres_data_by_tc(adres_cursor, anne_anne_tc)
            write_person_info(writer, anne_anne_result, prefix + "Büyükanne (Anne'nin Anne)", gsm_data, adres_data)
        if anne_baba_result:
            gsm_data = get_gsm_data_by_tc(gsm_cursor, anne_baba_tc)
            adres_data = get_adres_data_by_tc(adres_cursor, anne_baba_tc)
            write_person_info(writer, anne_baba_result, prefix + "Büyükbaba (Anne'nin Baba)", gsm_data, adres_data)
    ...
    if baba_result:
        ...
        adres_data = get_adres_data_by_tc(adres_cursor, baba_tc)
        write_person_info(writer, baba_result, prefix + "Baba", gsm_data, adres_data)
        ...
        if baba_anne_result:
            ...
            adres_data = get_adres_data_by_tc(adres_cursor, baba_anne_tc)
            write_person_info(writer, baba_anne_result, prefix + "Büyükanne (Baba'nın Anne)", gsm_data, adres_data)
        if baba_baba_result:
            gsm_data = get_gsm_data_by_tc(gsm_cursor, baba_baba_tc)
            adres_data = get_adres_data_by_tc(adres_cursor, baba_baba_tc)
            write_person_info(writer, baba_baba_result, prefix + "Büyükbaba (Baba'nın Baba)", gsm_data, adres_data)
    ...
    if cocuklari_result:
        for cocuk in cocuklari_result:
            ...
            adres_data = get_adres_data_by_tc(adres_cursor, cocuk[1])
            write_person_info(writer, cocuk, prefix + "-Çocuk", gsm_data, adres_data)
            ...
            if torunlari_result:
                for torun in torunlari_result:
                    ...
                    adres_data = get_adres_data_by_tc(adres_cursor, torun[1])
                    write_person_info(writer, torun, prefix + f"{cocuk[2]} {cocuk[3]}'in Çocuğu", gsm_data, adres_data)
    ...
    if kardesleri_result:
        for kardes in kardesleri_result:
            ...
            adres_data = get_adres_data_by_tc(adres_cursor, kardes[1])
            write_person_info(writer, kardes, prefix + "--Kardeşi", gsm_data, adres_data)
            ...
            if yegenleri_result:
                ...
                for yegen in yegenleri_result:
                    ...
                    adres_data = get_adres_data_by_tc(adres_cursor, yegen[1])
                    write_person_info(writer, yegen, prefix + "Yeğeni", gsm_data, adres_data)
                    ...
                    if yegenin_cocugu_result:
                        for yegenin_cocugu in yegenin_cocugu_result:
                            ...
                            adres_data = get_adres_data_by_tc(adres_cursor, yegenin_cocugu[1])
                            write_person_info(writer, yegenin_cocugu, prefix + f"{yegen[2]} {yegen[3]}'in Çocuğu", gsm_data, adres_data)
    ...
    if anne_result:
        ...
        if dayı_teyze_result:
            ...
            for dayı_teyze in dayı_teyze_result:
                ...
                adres_data = get_adres_data_by_tc(adres_cursor, dayı_teyze[1])
                write_person_info(writer, dayı_teyze, prefix + "---Dayı/Teyze", gsm_data, adres_data)
    if baba_result:
        ...
        if amca_hala_result:
            ...
            for amca_hala in amca_hala_result:
                ...
                adres_data = get_adres_data_by_tc(adres_cursor, amca_hala[1])
                write_person_info(writer, amca_hala, prefix + "---Amca/Hala", gsm_data, adres_data)
    ...
    if kuzenleri_result:
        for kuzen in kuzenleri_result:
            ...
            adres_data = get_adres_data_by_tc(adres_cursor, kuzen[1])
            write_person_info(writer, kuzen, prefix + "Kuzen", gsm_data, adres_data)
            ...
            if kuzen_cocuklari_result:
                for kuzen_cocugu in kuzen_cocuklari_result:
                    ...
                    adres_data = get_adres_data_by_tc(adres_cursor, kuzen_cocugu[1])
                    write_person_info(writer, kuzen_cocugu, prefix + f"{kuzen[2]} {kuzen[3]}'in Çocuğu", gsm_data, adres_data)
...
def process_tc_number(tc_no):
    db_config, gsm_config, adres_config = read_config()
    ...
    adres_cnx = connect_to_database(adres_config)
    if not adres_cnx:
        messagebox.showerror("Hata", "Adres veritabanına bağlanılamadı.")
        cnx.close()
        gsm_cnx.close()
        return
    ...
    if main_person:
        ...
        with open(filename, "w", encoding="utf-8", newline='') as file:
            ...
            writer.writerow(["Kategori", "TC", "Adı", "Soyadı", "Doğum Tarihi", "Nufus İli", "Nufus İlçesi", "Anne Adı", "Anne TC", "Baba Adı", "Baba TC", "Uyruk", "GSM", "GUNCELADRES", "GECMISADRES"])

            # Ana kişinin kendi sülalesi
            writer.writerow([])
            writer.writerow([f"Kendi Sülalesi"])
            summary_data_kendi = process_family_tree(cursor, gsm_cursor, adres_cursor, tc_no, writer, f"")
            ...
            if anne_tc:
                ...
                summary_data_anne = process_family_tree(cursor, gsm_cursor, adres_cursor, anne_tc, writer, f"")
            ...
            if baba_tc:
                ...
                summary_data_baba = process_family_tree(cursor, gsm_cursor, adres_cursor, baba_tc, writer, f"")
