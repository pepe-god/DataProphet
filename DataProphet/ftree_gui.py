...
def process_family_tree(cursor, tc_no, writer, prefix=""):
    ...
    write_person_info(writer, main_person, prefix + "Ana Kayıt")

    # Anne bilgileri
    anne_tc = main_person[8]
    anne_result = get_family_member_by_tc(cursor, anne_tc)
    if anne_result:
        write_person_info(writer, anne_result, prefix + "Anne")
        ...
        if anne_anne_result:
            write_person_info(writer, anne_anne_result, prefix + "Büyükanne (Anne'nin Anne)")
        if anne_baba_result:
            write_person_info(writer, anne_baba_result, prefix + "Büyükbaba (Anne'nin Baba)")

    # Baba bilgileri
    baba_tc = main_person[10]
    baba_result = get_family_member_by_tc(cursor, baba_tc)
    if baba_result:
        write_person_info(writer, baba_result, prefix + "Baba")
        ...
        if baba_anne_result:
            write_person_info(writer, baba_anne_result, prefix + "Büyükanne (Baba'nın Anne)")
        if baba_baba_result:
            write_person_info(writer, baba_baba_result, prefix + "Büyükbaba (Baba'nın Baba)")

    # Çocukları
    cocuklari_result = get_children_by_parent_tc(cursor, main_person[1])
    if cocuklari_result:
        for cocuk in cocuklari_result:
            write_person_info(writer, cocuk, prefix + "-Çocuk")

            # Torunları
            torunlari_result = get_children_by_parent_tc(cursor, cocuk[1])
            if torunlari_result:
                for torun in torunlari_result:
                    write_person_info(writer, torun, prefix + f"{cocuk[2]} {cocuk[3]}'in Çocuğu")
                ...
                for yegen in yegenleri_result:
                    ...
                    if yegenin_cocugu_result:
                        for yegenin_cocugu in yegenin_cocugu_result:
                            write_person_info(writer, yegenin_cocugu, prefix + f"{yegen[2]} {yegen[3]}'in Çocuğu")
    ...
    if anne_result:
        ...
        if dayı_teyze_result:
            ...
            for dayı_teyze in dayı_teyze_result:
                write_person_info(writer, dayı_teyze, prefix + "---Dayı/Teyze")
    if baba_result:
        ...
        if amca_hala_result:
            ...
            for amca_hala in amca_hala_result:
                write_person_info(writer, amca_hala, prefix + "---Amca/Hala")
    ...
    if kuzenleri_result:
        for kuzen in kuzenleri_result:
            write_person_info(writer, kuzen, prefix + "Kuzen")
            ...
            if kuzen_cocuklari_result:
                for kuzen_cocugu in kuzen_cocuklari_result:
                    write_person_info(writer, kuzen_cocugu, prefix + f"{kuzen[2]} {kuzen[3]}'in Çocuğu")
...
def process_tc_number(tc_no):
    ...
    if main_person:
        ...
        with open(filename, "w", encoding="utf-8", newline='') as file:
            ...
            summary_data_kendi = process_family_tree(cursor, tc_no, writer, f"")
            ...
            if anne_tc:
                ...
                summary_data_anne = process_family_tree(cursor, anne_tc, writer, f"")
            ...
            if baba_tc:
                ...
                summary_data_baba = process_family_tree(cursor, baba_tc, writer, f"")
